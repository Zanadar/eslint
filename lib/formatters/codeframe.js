...
function formatSummary(errors, warnings, fixableErrors, fixableWarnings) {
    ...
    if (fixableErrors > 0) {
        fixablesSummay.push(`${fixableErrors} ${pluralize("error", fixableErrors)}`);
    }

    if (fixableWarnings > 0) {
        fixablesSummay.push(`${fixableWarnings} ${pluralize("warning", fixableWarnings)}`);
    }
    ...
    if (fixableErrors || fixableWarnings) {
        output += chalk[summaryColor].bold(`\n${fixablesSummay.join(" and ")} potentially fixable with the \`--fix\` option.`);
    }
